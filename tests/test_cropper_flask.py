...
class CropperFlaskTest:
    ...
    def test_process_jpeg(self, image_path="test_image.jpg"):
        ...
        try:
            ...
            # Vérifiez le statut HTTP ET le champ "status" dans le JSON
            if response.status_code == 200:
                try:
                    data = response.json()
                    if data.get("status") == "success":
                        self.log("PASS", "/process request succeeded")
                        return True
                    else:
                        self.log("FAIL", f"/process returned non-success status: {data.get('status')}")
                        return False
                except json.JSONDecodeError:
                    self.log("FAIL", f"/process returned non-JSON response: {response.text}")
                    return False
            else:
                self.log("FAIL", f"/process returned HTTP {response.status_code}: {response.text}")
                return False
    ...
    def test_process_pdf(self, pdf_path="test_document.pdf"):
        """Test /process-pdf endpoint with PDF"""
        print(f"\n📄 Testing /process-pdf...")
        ...
        try:
            ...
            if response.status_code == 200:
                try:
                    data = response.json()
                    if data.get("status") == "success":
                        self.log("PASS", "/process-pdf request succeeded")
                        return True
                    else:
                        self.log("FAIL", f"/process-pdf returned non-success status: {data.get('status')}")
                        return False
                except json.JSONDecodeError:
                    self.log("FAIL", f"/process-pdf returned non-JSON response: {response.text}")
                    return False
            else:
                self.log("FAIL", f"/process-pdf returned HTTP {response.status_code}: {response.text}")
                return False
    ...
    def test_process_tiff(self, tiff_path="test_image.tiff"):
        """Test /process-tiff endpoint with TIFF"""
        ...
        try:
            ...
            if response.status_code == 200:
                try:
                    data = response.json()
                    if data.get("status") == "success":
                        self.log("PASS", "/process-tiff request succeeded")
                        return True
                    else:
                        self.log("FAIL", f"/process-tiff returned non-success status: {data.get('status')}")
                        return False
                except json.JSONDecodeError:
                    self.log("FAIL", f"/process-tiff returned non-JSON response: {response.text}")
                    return False
            else:
                self.log("FAIL", f"/process-tiff returned HTTP {response.status_code}: {response.text}")
                return False
